...
import org.opensearch.dataprepper.plugins.source.kinesis.converter.StreamRecordConverter;
...
public class ShardConsumer implements Runnable {
    ...
    private ShardConsumer(Builder builder) {
        ...
        recordConverter = new StreamRecordConverter(bufferAccumulator, builder.pluginMetrics, builder.streamConfig);
        ...
        this.kinesisSourceAggregateMetrics = builder.kinesisSourceAggregateMetrics;
    }

    public static Builder builder(final KinesisAsyncClient kinesisClient,
                                  final PluginMetrics pluginMetrics,
                                  final KinesisSourceAggregateMetrics kinesisSourceAggregateMetrics,
                                  final Buffer<Record<Event>> buffer,
                                  final KinesisStreamConfig streamConfig) {
        return new Builder(kinesisClient, pluginMetrics, kinesisSourceAggregateMetrics, buffer, streamConfig);
    }
    ...
    static class Builder {
        ...
        private KinesisStreamConfig streamConfig;

        public Builder(final KinesisAsyncClient kinesisClient,
                       final PluginMetrics pluginMetrics,
                       final KinesisSourceAggregateMetrics kinesisSourceAggregateMetrics,
                       final Buffer<Record<Event>> buffer,
                       final KinesisStreamConfig streamConfig) {
...
//        }
    ...
    public void run() {
        ...
        int interval;
        List<software.amazon.awssdk.services.kinesis.model.Record> records;
        ...
        while (!shouldStop) {
            ...
            if (!response.records().isEmpty()) {
                ...
                sequenceNumber = response.records().get(response.records().size() - 1).sequenceNumber();
                Instant lastEventTime = response.records().get(response.records().size() - 1).approximateArrivalTimestamp();
                ...
                records = response.records().stream()
                        .filter(record -> record.approximateArrivalTimestamp().isAfter(startTime))
    ...
    private GetRecordsResponse callGetRecords(String shardIterator) {
        ...
        try {
            ...
            CompletableFuture<GetRecordsResponse> response = kinesisClient.getRecords(req);
            return response.get();
        } catch(final InternalServerErrorException ex) {
            kinesisSourceAggregateMetrics.getStream5xxErrors().increment();
